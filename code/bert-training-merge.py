...
def preprocess_data():
    min_num = 999999
    df1 = pd.read_csv("../docs/llmgan/all_llm_gan_merge.csv", encoding="utf_8_sig")
    ...
    for t in range(2):
        for i in range(5):
            if i + 1 != 3:
                if min_num > len(df[(df["rating"] == i + 1) & (df["type"] == t + 1)]):
                    min_num = len(df[(df["rating"] == i + 1) & (df["type"] == t + 1)])
            else:
                if min_num > math.floor(len(df[(df["rating"] == i + 1) & (df["type"] == t + 1)]) / 2):
                    min_num = math.floor(len(df[(df["rating"] == i + 1) & (df["type"] == t + 1)]) / 2)

    clean_df = pd.DataFrame()
    for t in range(2):
        for i in range(5):
            if i + 1 <= 2:
                if len(df[(df["rating"] == i + 1) & (df["origin"] == 1) & (df["type"] == t + 1)]) < min_num:
                    clean_df = pd.concat([clean_df, df[(df["rating"] == i + 1) & (df["origin"] == 1) & (df["type"] == t + 1)]])
                    clean_df = pd.concat([clean_df, df[(df["rating"] == i + 1) & (df["origin"] == 0) & (df["type"] == t + 1)].sample(n=min_num-len(df[(df["rating"] == i + 1) & (df["origin"] == 1) & (df["type"] == t + 1)]))])
                else:
                    clean_df = pd.concat([clean_df, df[(df["rating"] == i + 1) & (df["origin"] == 1) & (df["type"] == t + 1)].sample(n=min_num)])
            elif i + 1 == 3:
                clean_df = pd.concat([clean_df, df[(df["rating"] == i + 1) & (df["type"] == t + 1)].sample(n=min_num * 2)])
            else:
                clean_df = pd.concat([clean_df, df[(df["rating"] == i + 1) & (df["type"] == t + 1)].sample(n=min_num)])
    ...
    conditions = [
        (target_df['status'] == -1) & (target_df['type'] == 1),
        (target_df['status'] == 0) & (target_df['type'] == 1),
        (target_df['status'] == 1) & (target_df['type'] == 1),
        (target_df['status'] == -1) & (target_df['type'] == 2),
        (target_df['status'] == 0) & (target_df['type'] == 2),
        (target_df['status'] == 1) & (target_df['type'] == 2),
    ]
    ...
    target_df = shuffle(target_df).reset_index(drop=True) 
    # print(labels)

    np.random.seed(random_seed)
    df_train, df_val, df_test = np.split(target_df, [int(.8*len(target_df)), int(.9*len(target_df))])
    print(len(df_train),len(df_val), len(df_test))
    pd.DataFrame(target_df, columns=["content", "status", "type", "label"]).to_csv("../model/gan_type1/target_df.csv", index=False, encoding='utf-8-sig')
