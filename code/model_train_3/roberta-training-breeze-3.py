...
def save_model(model, save_name):
    torch.save(model.state_dict(), f'new_data/docs_0819/Final_Breeze/Type1_Result/RoBERTa/{NUM_LABELS}/{save_name}')
...
def evaluate(dataset):
    # 加载模型
    model = RobertaClassifier()
    model.load_state_dict(torch.load(f'new_data/docs_0819/Final_Breeze/Type1_Result/RoBERTa/{NUM_LABELS}/best.pt'))
    model = model.to(device)
    model.eval()
...
def draw_loss_image(loss_list, loss_val_list):
    ...
    plt.xlabel('Epoches')
    plt.legend()
    plt.savefig(f"new_data/docs_0819/Final_Breeze/Type1_Result/RoBERTa/{NUM_LABELS}/RoBERTa_Loss.jpg")
...
def draw_acc_image(accuracy_list, accuracy_val_list):
    ...
    plt.xlabel('Epoches')
    plt.legend()
    plt.savefig(f"new_data/docs_0819/Final_Breeze/Type1_Result/RoBERTa/{NUM_LABELS}/RoBERTa_Acc.jpg")
...
def show_confusion_matrix(y_true, y_pred, class_num, fname, epoch):
    ...
    plt.savefig(fname=f"new_data/docs_0819/Final_Breeze/Type1_Result/RoBERTa/{NUM_LABELS}/{fname}.jpg")


def save_result(text, write_type):
    file_path = f"new_data/docs_0819/Final_Breeze/Type1_Result/RoBERTa/{NUM_LABELS}/result.txt"
