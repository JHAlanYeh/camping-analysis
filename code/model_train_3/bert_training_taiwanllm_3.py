...
df_train = pd.read_csv("new_data/docs_0819/Final_TaiwanLLM/Type1_Result/taiwanllm_type1_train_df.csv")
df_val = pd.read_csv("new_data/docs_0819/Final_TaiwanLLM/Type1_Result/type1_val_df.csv")
df_test = pd.read_csv("new_data/docs_0819/Final_TaiwanLLM/Type1_Result/type1_test_df.csv")
...
def save_model(model, save_name):
    torch.save(model.state_dict(), f'new_data/docs_0819/Final_TaiwanLLM/Type1_Result/BERT/{NUM_LABELS}/{save_name}')
...
def evaluate(dataset):
    ...
    # 加载模型
    model = BertClassifier()
    model.load_state_dict(torch.load(f'new_data/docs_0819/Final_TaiwanLLM/Type1_Result/BERT/{NUM_LABELS}/best.pt'))
    model = model.to(device)
    model.eval()
...
def preprocess_data():
    df = pd.read_csv("new_data/docs_0819/Final_TaiwanLLM/type1_comments_origin.csv", encoding="utf_8_sig")
    target_df = df
    ...
    pd.DataFrame(df_train, columns=["content", "rating", "status", "type", "label", "sequence_num", "publishedDate", "origin", "text"]).to_csv("new_data/docs_0819/Final_TaiwanLLM/type1_train_df.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(df_val, columns=["content", "rating", "status", "type", "label", "sequence_num", "publishedDate", "origin", "text"]).to_csv("new_data/docs_0819/Final_TaiwanLLM/type1_val_df.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(df_test, columns=["content", "rating", "status", "type", "label", "sequence_num", "publishedDate", "origin", "text"]).to_csv("new_data/docs_0819/Final_TaiwanLLM/type1_test_df.csv", index=False, encoding="utf-8-sig")
...
def draw_loss_image(loss_list, loss_val_list):
    ...
    plt.xlabel('Epoches')
    plt.legend()
    plt.savefig(f"new_data/docs_0819/Final_TaiwanLLM/Type1_Result/BERT/{NUM_LABELS}/BERT_Loss.jpg")
...
def draw_acc_image(accuracy_list, accuracy_val_list):
    ...
    plt.xlabel('Epoches')
    plt.legend()
    plt.savefig(f"new_data/docs_0819/Final_TaiwanLLM/Type1_Result/BERT/{NUM_LABELS}/BERT_Acc.jpg")
...
def show_confusion_matrix(y_true, y_pred, class_num, fname, epoch):
    ...
    plt.savefig(fname=f"new_data/docs_0819/Final_TaiwanLLM/Type1_Result/BERT/{NUM_LABELS}/{fname}.jpg")


def save_result(text, write_type):
    file_path = f"new_data/docs_0819/Final_TaiwanLLM/Type1_Result/BERT/{NUM_LABELS}/result.txt"
