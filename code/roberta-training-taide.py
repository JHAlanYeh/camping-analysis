...
def save_model(model, save_name):
    torch.save(model.state_dict(), f'new_data/docs_0724/Final_TAIDE/Type1_Result/RoBERTa/{save_name}')
...
def evaluate(dataset):
    ...
    # 加载模型
    model = RobertaClassifier()
    model.load_state_dict(torch.load('new_data/docs_0724/Final_TAIDE/Type1_Result/RoBERTa/best.pt'))
    model = model.to(device)
    model.eval()
...
def draw_loss_image(loss_list, loss_val_list):
    ...
    plt.xlabel('Epoches')
    plt.legend()
    plt.savefig("new_data/docs_0724/Final_TAIDE/Type1_Result/RoBERTa/RoBERTa_Loss.jpg")
...
def draw_acc_image(accuracy_list, accuracy_val_list):
    ...
    plt.xlabel('Epoches')
    plt.legend()
    plt.savefig("new_data/docs_0724/Final_TAIDE/Type1_Result/RoBERTa/RoBERTa_Acc.jpg")
...
def show_confusion_matrix(y_true, y_pred, class_num, fname, epoch):
    ...
    plt.savefig(fname=f"new_data/docs_0724/Final_TAIDE/Type1_Result/RoBERTa/{fname}.jpg")


def save_result(text, write_type):
    file_path = "new_data/docs_0724/Final_TAIDE/Type1_Result/RoBERTa/result.txt"
