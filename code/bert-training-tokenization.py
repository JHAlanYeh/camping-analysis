...
def preprocess_data():
    min_num = 999999
    df1 = pd.read_csv("../docs/gan/type1_gan_merge_tokenization.csv", encoding="utf_8_sig")
    ...
    target_df = clean_df[["tokenization", "status", "type"]]
...
if __name__ == "__main__":
    ...
    # pd.DataFrame(df_train, columns=["tokenization", "status", "type", "label"]).to_csv("../model/gan_type1/train_df.csv", index=False, encoding='utf-8-sig')
    # pd.DataFrame(df_val, columns=["tokenization", "status", "type", "label"]).to_csv("../model/gan_type1/val_df.csv", index=False, encoding='utf-8-sig')
    # pd.DataFrame(df_test, columns=["tokenization", "status", "type", "label"]).to_csv("../model/gan_type1/test_df.csv", index=False, encoding='utf-8-sig')
